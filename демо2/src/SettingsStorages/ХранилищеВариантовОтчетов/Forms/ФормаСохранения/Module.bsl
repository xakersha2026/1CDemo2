...
&НаКлиенте
Асинх Процедура ВыбратьНастройкуСохранения()
	...
	Если КодСохраняемойНастройки <> Неопределено Тогда
		...
		РезультатЗаменыНастройки = Ждать ВопросАсинх(ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Если РезультатЗаменыНастройки = КодВозвратаДиалога.Да Тогда
...
&НаКлиенте
Асинх Процедура УдалениеНастройку()
	РезультатВопросаОбУдалении = Ждать ВопросАсинх(НСтр("ru = ""Удалить вариант """) + Элементы.СписокНастроек.ТекущиеДанные.Наименование + "?", РежимДиалогаВопрос.ДаНет);
	Если РезультатВопросаОбУдалении = КодВозвратаДиалога.Да Тогда
