...
// если Неопределено, то возвращает список настроек по умолчанию
...
Процедура ПолучитьСписокСохраненныхНастроек(Пользователь, СписокФормССохраненнымиНастройками) Экспорт
	...
	Перем Представление;
	Перем Картинка;
	Перем ВыборкаНастроек;
	
	ТипНастроек = "/НастройкиФормы";
	ДлинаСтрокиТипаНастроек = СтрДлина(ТипНастроек);
	
	Если Пользователь = Неопределено Тогда
		ВыборкаНастроек = ХранилищеСистемныхНастроек.ВыбратьНастройкиПоУмолчанию();
	Иначе
...
Процедура СкопироватьНастройкиФорм(ПользовательИсточник, ПользователиПриемник, МассивНастроекДляКопирования) Экспорт
	...
	Если ПользовательИсточник = Неопределено ИЛИ ПользователиПриемник = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура УдалитьНастройкиФорм(Пользователь, МассивНастроекДляУдаления) Экспорт
	...
	Если Пользователь = Неопределено Тогда
		Для каждого Элемент Из МассивНастроекДляУдаления Цикл
			...
			ХранилищеСистемныхНастроек.УдалитьНастройкуПоУмолчанию(Элемент + "/НастройкиФормы");
...
// Процедура сохраняет настройки форм заданного пользователя в качестве настроек по умолчанию
//
// Параметры :
//  ПользовательИсточник - имя пользователя, настройки форм которого сохраняются в качестве настроек по умолчанию
//  МассивНастроек - имена форм, настройки которых нужно сохранить в качестве настроек по умолчанию
Процедура СохранитьПоУмолчанию(ПользовательИсточник, МассивНастроек) Экспорт

	Если ПользовательИсточник = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого Элемент Из МассивНастроек Цикл
		...
		Если Настройка <> Неопределено Тогда
			...
			ХранилищеСистемныхНастроек.СохранитьНастройкуПоУмолчанию(Элемент + "/НастройкиФормы", Настройка);
