...
&НаКлиенте
Асинх Процедура КопироватьВБуферОбмена(Команда)
	Если НЕ СредстваБуфераОбмена.ИспользованиеДоступно() Тогда
		ПоказатьОповещениеПользователя(,, НСтр("ru='Работа с буфером обмена недоступна. Обратитесь к администратору.'", "ru"), БиблиотекаКартинок.ДиалогВосклицание);
		Возврат;
	КонецЕсли;
	
	СтрокаВБуферОбмена = СтрШаблон(НСтр("ru='Заказ N%1'", "ru"), Объект.Номер);
	Для Каждого ТекущаяСтрока из Объект.Товары Цикл 
		ИнформацияПоТовару = СтрШаблон("%1 %2 %3", ТекущаяСтрока.Количество, ТекущаяСтрока.Товар, Формат(ТекущаяСтрока.Сумма,"ЧГ="));
		СтрокаВБуферОбмена = СтрокаВБуферОбмена + Символы.ПС + ИнформацияПоТовару ;
	КонецЦикла;
	
	РезультатВставки = Ждать СредстваБуфераОбмена.ПоместитьДанныеАсинх(Новый ЭлементБуфераОбмена(СтандартныйФорматДанныхБуфераОбмена.Текст, СтрокаВБуферОбмена));
	Если РезультатВставки Тогда
		ПоказатьОповещениеПользователя(,, НСтр("ru='Информация по заказу скопирована в буфер обмена'", "ru"), БиблиотекаКартинок.ДиалогИнформация);
	Иначе
		ПоказатьОповещениеПользователя(,, НСтр("ru='Ошибка помещения данных в буфер обмена'", "ru"), БиблиотекаКартинок.ДиалогИнформация);
	КонецЕсли;
КонецПроцедуры
