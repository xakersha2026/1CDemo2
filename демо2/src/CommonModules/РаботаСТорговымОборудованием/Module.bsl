...
Асинх Процедура ВыполнитьПодключениеДрайвераСканераШтрихкодов()
	...
	Если ПараметрыСканера = Неопределено Тогда
		...
		Возврат;
	КонецЕсли;
	ДрайверСканераШтрихкодов = Ждать СоздатьОбъектВнешнейКомпонентыАсинх("AddIn.Сканер.BarcodeReader");
	Если ДрайверСканераШтрихкодов = Неопределено Тогда
		Ждать ПредупреждениеАсинх(НСтр("ru='Ошибка создания объекта компоненты драйвера сканера штрих-кодов'", "ru"));
		Возврат;		
	КонецЕсли;
